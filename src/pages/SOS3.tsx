  ...
  return (
    <IonPage className="bg-white">
      ...
      <div className="flex flex-col text-black font-faculty h-screen items-center space-y-2 p-2">
        <h1 className="font-bold">Add Car Number Photo</h1>
        ...
        <div className='flex space-x-3 items-center overflow-x-auto w-full p-2'>
          ...
          <input type="text" value={carNo} onChange={(e:any)=>{
            setCarNo(e.target.value)
          }} className="bg-white border-2 border-gray-400 rounded-lg p-3" placeholder="Enter Vehicle Number"></input>
        
        {photos.length > 0 && carNo!=="" &&(
          <button className="bg-blue-500 rounded-lg p-2 text-white mt-4" onClick={()=>{router.push("/sos4")}}>Proceed</button>
        )}
      </div>
    </IonPage>
  );
